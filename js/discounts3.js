const countBuyValues__dis__k = [3];
const prices__dis__k = [316];

const countContainers__dis__k = document.querySelectorAll('.basket__buy__count__k');

countContainers__dis__k.forEach((container, index) => {
    ...
    plusButton.addEventListener('click', () => {
        if (count__product < countBuyValues__dis__k) {
            count__product++;
            updateButtonsState();
        }
    });
    ...
    function updateButtonsState() {
        ...
        const totalPrice__dis__k = count__product * prices__dis__k;

        let outputElement__dis__k = document.getElementById("price__present__dis__k");

        outputElement__dis__k.textContent = totalPrice__dis__k;

        let outputElement__dis__k__mobile = document.getElementById("price__present__dis__k__mobile");

        outputElement__dis__k__mobile.textContent = totalPrice__dis__k;
        ...
        console.log(`Total price for product : ${totalPrice__dis__k}`);
    }
});
